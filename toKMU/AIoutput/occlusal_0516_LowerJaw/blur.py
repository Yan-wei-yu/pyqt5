...
# 載入修補後的圖片
depth_fixed = cv2.imread('./patched_resultV1.png', cv2.IMREAD_GRAYSCALE)

# 建立遮罩：手動框選修補區域 + dilate 擴展出過渡邊界
mask = cv2.imread('./mask_boundary.png', 0)  # 二值遮罩圖，中間缺損為255，其餘為0
...
cv2.imwrite('./tworestore.png', depth_result)
...
# # 讀入灰階遮罩
# mask_gray = cv2.imread("./original_edge.png", cv2.IMREAD_GRAYSCALE)
